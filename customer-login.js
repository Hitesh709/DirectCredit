...
function renderDashboard() {
  const c = profileData?.customer || currentCustomer || {}, m = profileData?.metrics || {}, loans = profileData?.loans || [], repayments = profileData?.repayments || [], journey = profileData?.journey || [];
  ...
  const scoreEl = document.querySelector('.welcome-score strong'), scoreStatus = document.querySelector('.welcome-score span');
...
function renderJourney(rows) {
  const host = document.getElementById('homeProgress'), rail = document.getElementById('journeyRail');
  ...
  const current=sorted.find(x=>String(x.status).toLowerCase()==='current')||sorted.find(x=>String(x.status).toLowerCase()==='pending')||sorted[0], panel=document.getElementById('stepPanel');
  if(panel&&current)panel.innerHTML=`<div class="step-body"><span class="eyebrow">APPLICATION STEP</span><h2>${esc(text(current.step_label))}</h2><p>Status: <b>${esc(statusLabel(current.status))}</b></p></div>`;
}
function renderProfile(c) { const box=document.querySelector('.profile-data'); if(!box)return; const rows=[['Customer ID',c.customer_code||c.id],['Mobile Number',c.mobile],['Email',c.email],['Address',c.address],['Business Name',c.business_name],['Business Type',c.business_type],['Customer Type',c.customer_type],['Occupation',c.occupation],['Monthly Income',c.monthly_income==null?null:money(c.monthly_income)],['Primary Bank',c.primary_bank],['Average Bank Balance',c.average_bank_balance==null?null:money(c.average_bank_balance)],['CIBIL Score',c.cibil_score],['FOIR',c.foir==null?null:`${c.foir}%`],['Existing EMI',c.existing_emi==null?null:money(c.existing_emi)]]; box.innerHTML=rows.map(([k,v])=>`<div><span>${esc(k)}</span><b>${esc(text(v))}</b></div>`).join(''); }
function renderProfileSecondary(c,data) { const box=document.querySelector('.profile-data-secondary'); if(!box)return; const k=data?.kyc_employment||{}; const rows=[['KYC Status',k.kyc_status],['Employment / Occupation',k.employment_type],['Monthly Income',k.income==null?null:money(k.income)],['Work Experience',k.work_experience_years==null?null:`${k.work_experience_years} years`],['Years in Business',k.years_in_business==null?null:`${k.years_in_business} years`],['Residence Ownership',k.residence_ownership],['Ownership Proof',k.ownership_proof_status],['Credit Score',c.cibil_score],['FOIR',c.foir==null?null:`${c.foir}%`],['Existing EMI',c.existing_emi==null?null:money(c.existing_emi)]]; box.innerHTML=rows.map(([key,value])=>`<div><span>${esc(key)}</span><b>${esc(text(value))}</b></div>`).join(''); }
...
function renderRepayments(rows) { const body=document.querySelector('#repayment tbody'); if(!body)return; const paid=rows.reduce((s,r)=>s+Number(r.paid_amount||0),0), unpaid=rows.reduce((s,r)=>s+Math.max(Number(r.due_amount||0)-Number(r.paid_amount||0),0),0), metrics=[...document.querySelectorAll('#repayment .metrics .metric strong')]; if(metrics[0])metrics[0].textContent=money(paid); if(metrics[1])metrics[1].textContent=money(unpaid); const next=rows.filter(r=>Number(r.due_amount||0)>Number(r.paid_amount||0)).sort((a,b)=>String(a.due_date).localeCompare(String(b.due_date)))[0]; if(metrics[2])metrics[2].textContent=next?money(Math.max(Number(next.due_amount||0)-Number(next.paid_amount||0),0)):'Not available'; if(!rows.length){body.innerHTML='<tr><td colspan="5" class="empty-state">No repayment records found.</td></tr>';return;} body.innerHTML=rows.map(r=>`<tr><td>${esc(text(r.due_date))}</td><td>${esc(text(r.loan_id))}</td><td>${esc(money(r.paid_amount||r.due_amount))}</td><td>Recorded</td><td>${esc(statusLabel(r.status))}</td></tr>`).join(''); }
...
function bind(){document.getElementById('loginBtn')?.addEventListener('click',loginWithMobile);document.getElementById('loginId')?.addEventListener('keydown',e=>{if(e.key==='Enter')loginWithMobile();});document.getElementById('logoutBtn')?.addEventListener('click',logout);document.querySelectorAll('.side-nav').forEach(btn=>btn.addEventListener('click',()=>openSection(btn.dataset.section)));document.querySelectorAll('[data-open-section]').forEach(btn=>btn.addEventListener('click',()=>openSection(btn.dataset.openSection)));const mobile=sessionStorage.getItem(CUSTOMER_KEY),token=sessionStorage.getItem(TOKEN_KEY);if(mobile&&token)loadCustomerProfile(mobile).then(()=>{showPortal();openSection('home');}).catch(()=>logout());}
