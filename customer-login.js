...
const AUTH_LOGIN_PATH = '/auth/customer-mobile-login';
const AUTH_LOGIN_LEGACY_PATH = '/services/api/auth/customer-mobile-login';
...
async function loginWithMobile(){ const input=document.getElementById('loginId'),mobile=String(input?.value||'').replace(/\D/g,'').slice(0,10); if(mobile.length!==10){setLoginMessage('Enter a valid 10-digit mobile number.',true);return;} const button=document.getElementById('loginBtn'); if(button){button.disabled=true;button.textContent='Checking customer record…';} setLoginMessage(''); try{let result;
    try {
      result=await api(AUTH_LOGIN_PATH,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mobile})});
    } catch(primaryErr) {
      // Keep compatibility with older deployments while the canonical /api/auth route rolls out.
      if (!/404|Request failed \(5/.test(String(primaryErr.message||''))) throw primaryErr;
      result=await api(AUTH_LOGIN_LEGACY_PATH,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mobile})});
    } sessionStorage.setItem(TOKEN_KEY,result.access_token); sessionStorage.setItem(CUSTOMER_KEY,String(result.customer.id)); await loadCustomerProfile(result.customer.id); showPortal(); openSection('home');}catch(err){sessionStorage.removeItem(TOKEN_KEY);sessionStorage.removeItem(CUSTOMER_KEY);showLogin();setLoginMessage(err.message||'Customer record could not be loaded.',true);}finally{if(button){button.disabled=false;button.textContent='Enter Customer Portal';}} }
