...
window.DirectCreditData = (() => {
  ...
  async function reporting(){if(DEMO_MODE)return demoReporting();try{const d=await get('/admin/reporting');return {...d,source:'live'};}catch(e){if(DEMO_FALLBACK)return demoReporting();throw e}}
  async function loans(){if(DEMO_MODE)return DEMO_LOANS;try{const d=await get('/admin/reports/loan-pipeline');const rows=Array.isArray(d)?d:(d.rows||[]);return rows.map(x=>({...x,id:x.id??x.loan_id,requested_amount:x.requested_amount,eligible_amount:x.eligible_amount,sanctioned_amount:x.sanctioned_amount,disbursed_amount:x.disbursed_amount,outstanding_amount:x.outstanding_amount,monthly_emi:x.monthly_emi,tenure_months:x.tenure_months,interest_rate:x.interest_rate,customer_name:x.customer_name,customer_code:x.customer_code,mobile:x.mobile,business_name:x.business_name,status:x.status,current_stage:x.stage??x.current_stage}))}catch(e){if(DEMO_FALLBACK)return DEMO_LOANS;throw e}}
  ...
  async function customer(id, options={}){
    ...
    try {
      ...
      const paid=rows.reduce((s,x)=>s+Math.max(Number(x.disbursed_amount||0)-Number(x.outstanding_amount||0),0),0);
      const overdue=rows.filter(x=>x.status==='overdue').reduce((s,x)=>s+Number(x.outstanding_amount||0),0);
      const latest=rows[0]||{};
      return {
        customer:cr,
        loans:rows,
        metrics:{
          ...
          total_loan_amount:rows.reduce((s,x)=>s+Number(x.sanctioned_amount||x.requested_amount||0),0),
          outstanding_amount:outstanding,
          amount_paid:paid,
          overdue_amount:overdue
        },
        bank_analysis:{status:cr.primary_bank?'Customer banking field recorded':'No bank account data connected'},
        kyc_employment:{
        ...
        risk_score:{
          total_score:latest.scorecard_score??null,
          max_score:latest.scorecard_max??125,
          raw_score:latest.scorecard_score??null,
          risk_tier:latest.scorecard_decision==='REJECT'?'High Risk':latest.scorecard_score>=105?'Low Risk':latest.scorecard_score>=95?'Moderate Risk':'Not assessed',
          decision:latest.scorecard_decision||'Not assessed',
          credit_score:cr.cibil_score||null,
          source:latest.scorecard_score!=null?'scorecard':'scorecard_not_configured',
          scorecard_version:latest.scorecard_version||'MBL-125-v1',
          approval_percent:latest.scorecard_approval_percent??null,
          reasons:parsed(latest.scorecard_reasons)||[],
          hard_rejects:parsed(latest.scorecard_hard_rejects)||[],
          factor_scores:parsed(latest.scorecard_factor_scores)||{}
        },
        directcredit_score:latest.scorecard_score??null,
        source:'live'
      };
    } catch(e) {
      // Explicit demo fallback keeps the admin application workspace usable even when
      // the live API is unavailable. The UI labels the record as demo; live is always tried first.
      if(DEMO_FALLBACK) return demoCustomer(id);
      throw e;
    }
  }
  return {base,headers,reporting,customer,loans,demoCustomer,demoCustomers:DEMO_CUSTOMERS};
})();
