...
window.DirectCreditData = (() => {
  ...
  async function loans(){if(DEMO_MODE)return DEMO_LOANS;try{const d=await get('/admin/reports/loan-pipeline');const rows=Array.isArray(d)?d:(d.rows||[]);return rows.map(x=>({...x,id:x.id??x.loan_id,requested_amount:x.requested_amount,eligible_amount:x.eligible_amount,sanctioned_amount:x.sanctioned_amount,disbursed_amount:x.disbursed_amount,status:x.status,current_stage:x.stage??x.current_stage}))}catch(e){if(DEMO_FALLBACK)return DEMO_LOANS;throw e}}
  ...
  async function customer(id){
    ...
    // Customer detail is a protected admin read. Prefer the canonical Customer 360
    // profile aggregation so every customer tab uses the same real-data payload.
    try {
      ...
      // Backward-compatible fallback for deployments where the profile route is not
      // available yet; this still keeps the page live-first and never exposes data.
    }
