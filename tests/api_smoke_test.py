...
def test_foundation_smoke_flow():
    with TestClient(app) as client:
        ...
        loan = client.post("/api/loans", headers=headers, json={"customer_id": customer_id, "requested_amount": 10000, "tenure_months": 6})
        assert loan.status_code == 200
        loan_id = loan.json()["id"]
        assert client.post(f"/api/loans/{loan_id}/repayment-schedule", headers=headers).status_code == 200
        ...
        document = client.post("/api/documents", headers=headers, json={"customer_id": customer_id,"loan_id": loan_id,"document_type": "PAN","document_role": "identity","file_name": "pan.pdf","mime_type": "application/pdf","file_size": 1024})
