...
def seed_demo_data(db: Session) -> None:
    ...
    for i in range(10):
        ...
        customer = CustomerRecord(
            name=names[i],
            pan=f"DCDMO{i+1:04d}X",
            mobile=f"90000000{i+1:02d}",
            email=f"demo{i+1}@directcredit.test",
            address=f"Demo Business Address, {cities[i % len(cities)]}",
            current_city=cities[i % len(cities)],
            business_name=businesses[i],
            business_type="Proprietorship",
            date_of_birth=f"198{i % 10}-0{(i % 9) + 1}-15",
            aadhaar_masked=f"XXXX XXXX {1000 + i:04d}",
            customer_type="Individual",
            occupation="Self Employed",
            monthly_income=18000 + i * 2500,
            work_experience_years=4 + (i % 7),
            years_in_business=3 + (i % 8),
            average_bank_balance=12000 + i * 1800,
            primary_bank=["HDFC Bank", "ICICI Bank", "Axis Bank", "SBI", "Kotak Mahindra Bank"][i % 5],
            cibil_score=rng_scores[i],
            foir=35 + (i % 6) * 3,
            existing_emi=1800 + i * 250,
            dependents=1 + (i % 4),
            kyc_status="verified",
        ...
        loan = LoanRecord(
            ...
            outstanding_amount=0.0 if status == "repaid" else (round(amount * 0.72, 2) if status == "overdue" else (round(amount * 0.55, 2) if disbursed else 0.0)),
            interest_rate=15.0,
            tenure_months=6,
            status=status,
            current_stage={
