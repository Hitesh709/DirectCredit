...
def mask_mobile(value:Any)->str|None:
    ...
    s=str(value);return "*"*max(0,len(s)-4)+s[-4:]
def mask_pan(value:Any)->str|None:
    if value is None:return None
    s=str(value);return s[:2]+"*"*max(0,len(s)-4)+s[-2:] if len(s)>=4 else "***"
def mask_aadhaar(value:Any)->str|None:
    if value is None:return None
    s=re.sub(r"\D","",str(value));return "XXXX-XXXX-"+s[-4:] if len(s)>=4 else "XXXX-XXXX-XXXX"
def safe_event_details(value:Any)->Any:
    if isinstance(value,dict):return {k:"[REDACTED]" if str(k).lower() in SENSITIVE_KEYS else safe_event_details(v) for k,v in value.items()}
    if isinstance(value,list):return [safe_event_details(v) for v in value]
    return value
def validate_idempotency_key(value:str)->str:
    if not IDEMPOTENCY_RE.fullmatch(value or ""):raise ValueError("Invalid idempotency key")
    return value
def production_config_issues()->list[str]:
    issues=[];env=str(getattr(settings,"app_env",os.getenv("APP_ENV","development"))).lower()
    if env=="production":
        ...
        if len(str(getattr(settings,"directcredit_secret",os.getenv("DIRECTCREDIT_SECRET",""))))<32:issues.append("weak_or_missing_directcredit_secret")
    return issues
...
def config_drift(admin:dict=Depends(get_current_admin)):
    issues=production_config_issues();return {"production_safe":not issues,"issues":issues}
...
def audit_integrity(db:Session=Depends(get_db),admin:dict=Depends(get_current_admin)):
    ...
    return {"checked":len(rows),"valid":not invalid,"invalid_count":len(invalid),"invalid":invalid[:50]}
@router.get("/release-readiness")
def release_readiness(db:Session=Depends(get_db),admin:dict=Depends(get_current_admin)):
    drift=production_config_issues();audit=audit_integrity(db,admin);providers=provider_status()
    pending=[n for n,i in providers.items() if i.get("configured") is False and i.get("provider") not in {"internal"}]
    ...
    return {"ready_for_live_verification":all(checks[k] for k in checks if k!="live_deployment_verified"),"checks":checks,"config_issues":drift,"external_providers_pending":pending,"note":"Live deployment and real-provider credentials must be verified in the target environment; CI cannot claim them."}
