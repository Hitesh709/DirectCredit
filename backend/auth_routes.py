...
from .auth import hash_password, verify_password, issue_token, decode_token, revoke_token, get_current_customer
...
def register(payload: RegisterRequest, db: Session = Depends(get_db)):
    ...
    return {"customer":_customer_payload(c),"access_token":issue_token(c.id,"customer","access",1),"refresh_token":issue_token(c.id,"customer","refresh",30),"email_verification_token":verification_token}
...
def login(payload: LoginRequest, db: Session = Depends(get_db)):
    ...
    return {"access_token":issue_token(c.id,"customer","access",1),"refresh_token":issue_token(c.id,"customer","refresh",30),"token_type":"bearer","expires_in":3600,"customer":_customer_payload(c)}
...
def customer_mobile_login(payload: dict, db: Session = Depends(get_db)):
    ...
    No OTP/password is requested by the temporary customer portal. This endpoint
    never creates a customer and never accepts a browser-generated identity.
    """
    ...
    if not c: c=db.query(CustomerRecord).filter(CustomerRecord.mobile==f"+91{mobile}").first()
...
def logout(authorization: str | None = Header(default=None)):
    ...
    token=authorization.split(" ",1)[1].strip(); claims=decode_token(token)
    if not claims: raise HTTPException(401,"Invalid or expired token")
    revoke_token(token); return {"status":"logged_out"}
...
def reset_password(payload: PasswordRequest, db: Session = Depends(get_db)):
    ...
    c=db.get(CustomerRecord,claims["user_id"])
    if not c: raise HTTPException(404,"Customer not found")
    c.password_hash=hash_password(payload.new_password); db.commit(); return {"status":"password_reset"}
