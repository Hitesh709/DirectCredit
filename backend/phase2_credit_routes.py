"""Phase 2 Credit Decision + Fraud APIs."""
...
def run_assessment(customer_id: int, body: AssessRequest, db: Session = Depends(get_db), claims: dict = Depends(get_current_customer)):
    ...
    try: return assess(db, customer_id, body.loan_id, body.overrides)
...
def get_decision(customer_id: int, db: Session = Depends(get_db), claims: dict = Depends(get_current_customer)):
    ...
    if not loan: return {"customer_id": customer_id, "decision": "NOT_ASSESSED"}
    import json
    return {"customer_id": customer_id, "loan_id": loan.id, "decision": loan.scorecard_decision or "NOT_ASSESSED", "score": loan.scorecard_score, "max_score": loan.scorecard_max, "approval_percent": loan.scorecard_approval_percent, "eligible_amount": loan.eligible_amount, "reasons": json.loads(loan.scorecard_reasons or "[]"), "hard_rejects": json.loads(loan.scorecard_hard_rejects or "[]"), "factor_scores": json.loads(loan.scorecard_factor_scores or "{}"), "scorecard_version": loan.scorecard_version}
