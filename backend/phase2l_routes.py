...
class CallLog(BaseModel):
    disposition:str=Field(min_length=2,max_length=40); reference:str=Field(min_length=3,max_length=160); notes:str|None=Field(default=None,max_length=1500); callback_at:str|None=Field(default=None,max_length=30)
class VisitLog(BaseModel):
    outcome:str=Field(min_length=2,max_length=50); reference:str=Field(min_length=3,max_length=160); notes:str|None=Field(default=None,max_length=1500); latitude:float|None=None; longitude:float|None=None; visited_at:str|None=Field(default=None,max_length=30)
class AssignmentRequest(BaseModel):
    agent_id:int; reference:str=Field(min_length=3,max_length=160); notes:str|None=Field(default=None,max_length=1000)
...
def _audit(db,loan,customer,action,reference,notes):
    ...
    row=CollectionActionRecord(loan_id=loan.id,customer_id=customer.id,action_type=action,amount=0,reference=reference,status="recorded",notes=json.dumps(notes,ensure_ascii=False)); db.add(row); db.commit(); db.refresh(row); return row
def _priority(loan,db):
    rows=db.query(RepaymentRecord).filter(RepaymentRecord.loan_id==loan.id).all(); overdue,max_dpd=overdue_amount(rows); bucket=collection_bucket(max_dpd); p=build_priority(bucket,overdue,max_dpd); return overdue,max_dpd,p
@router.get("/contract")
...
def queue(db:Session=Depends(get_db),admin=Depends(get_current_admin)):
    ...
    for loan in db.query(LoanRecord).filter(LoanRecord.status.in_(["active","overdue"])).all():
        ...
        if overdue>0: out.append({"loan_id":loan.id,"customer_id":loan.customer_id,"dpd":max_dpd,"overdue_amount":overdue,"priority_score":p.score,"urgency":p.urgency})
...
def timeline(loan_id:int,db:Session=Depends(get_db),admin=Depends(get_current_admin)):
    loan,_=_loan(loan_id,db); rows=db.query(CollectionActionRecord).filter(CollectionActionRecord.loan_id==loan_id,CollectionActionRecord.action_type.in_(["agent_assignment","collection_call","field_visit"])).order_by(CollectionActionRecord.id.desc()).all()
