...
app = FastAPI(title="DirectCredit API", version="0.7.1")
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    for item in exc.errors():
        ...
        fields.append({"field": loc or "request", "message": item.get("msg", "Invalid value"), "type": item.get("type", "validation_error")})
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    # Preserve known business/lifecycle messages while keeping authentication errors generic.
...
async def unhandled_exception_handler(request: Request, exc: Exception):
    # Never expose exception text, SQL, credentials, tokens, or stack traces to API clients.
...
@app.get("/")
def root(): return {"application":"DirectCredit","status":"online","mode":"MVP","api_version":"0.7.1"}
@app.get("/health")
def health(db: Session = Depends(get_db)):
    db.execute(func.count(CustomerRecord.id)); return {"status":"healthy","database":"connected"}
@app.get("/api/version")
def version(): return {"name":"DirectCredit API","version":"0.7.1"}
...
def customer_login(payload: CustomerLogin, db: Session = Depends(get_db)):
    ...
    if customer:
        ...
        elif not verify_password(payload.password, customer.password_hash): raise HTTPException(401, "Invalid customer ID or password")
    else:
