...
app = FastAPI(title="DirectCredit API", version="0.7.2")
...
async def http_exception_handler(request: Request, exc: HTTPException):
    code, safe_message = public_http_error(exc.status_code)
    message = safe_message if exc.status_code in {401,403} else str(exc.detail)
    details = None if exc.status_code in {401,403,404,500} else exc.detail
...
async def unhandled_exception_handler(request: Request, exc: Exception):
    return JSONResponse(status_code=500, content=api_error("INTERNAL_ERROR", "An unexpected server error occurred.", request_id_value=request.state.request_id))
...
def assert_customer_access(customer_id: int, claims: dict):
    if int(claims.get("user_id", -1)) != int(customer_id): raise HTTPException(status_code=403, detail="Customer session does not match this customer")
...
def audit_http(request: Request, db: Session, *, action: str, entity_type: str, entity_id=None, customer_id=None, loan_id=None, claims=None, outcome="success", reason_code=None, details=None):
    ...
    record_event(db, action=action, entity_type=entity_type, entity_id=entity_id, customer_id=customer_id, loan_id=loan_id, outcome=outcome, reason_code=reason_code, details=details, **ctx)
...
@app.get("/")
def root(): return {"application":"DirectCredit","status":"online","mode":"MVP","api_version":"0.7.2"}
@app.get("/health")
def health(db: Session = Depends(get_db)):
    db.execute(func.count(CustomerRecord.id)); return {"status":"healthy","database":"connected"}
@app.get("/api/version")
def version(): return {"name":"DirectCredit API","version":"0.7.2"}
...
def create_customer(payload: CustomerCreate, request: Request, db: Session = Depends(get_db)):
    ...
    audit_http(request, db, action="CUSTOMER_CREATED", entity_type="customer", entity_id=c.id, customer_id=c.id, details={"customer_code":c.customer_code}); db.commit()
    return c
