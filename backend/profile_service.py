...
def profile_payload(customer_id: int, db: Session) -> dict:
    ...
    bank = {
        "total_transactions": 0, "upi_transactions": 0, "last_balance": c.average_bank_balance or 0,
        "average_eod_balance": c.average_bank_balance or 0, "average_monthly_credit": c.monthly_income or 0,
        "average_monthly_debit": c.existing_emi or 0, "monthly_breakdown": []
    }
    risk_score = min(100, max(0, round((c.cibil_score or 0) / 9 + max(0, 20 - (c.foir or 0) / 2), 1)))
    risk = {
        "total_score": risk_score, "risk_tier": "Low" if risk_score >= 75 else "Medium" if risk_score >= 55 else "High",
        "decision": "Eligible" if risk_score >= 55 else "Review", "age": None, "bank_verification": c.kyc_status,
        "credit_score": c.cibil_score, "credit_enquiries": None, "negative_balance": None,
        "monthly_income": c.monthly_income, "bank_vintage": None, "dpd_analysis": None,
        "debt_analysis": None, "cheque_return": None
    }
    return {
        ...
        "metrics": {"total_loans": len(loans), "total_loan_amount": sanctioned, "outstanding_amount": outstanding, "amount_paid": paid, "credit_score": c.cibil_score, "overdue_amount": overdue},
        ...
        "documents": [{"id":d.id,"loan_id":d.loan_id,"document_type":d.document_type,"file_name":d.file_name,"verification_status":d.verification_status,"created_at":str(d.created_at) if d.created_at else None} for d in docs],
        "bank_analysis": bank,
        "kyc_employment": {"kyc_status": c.kyc_status, "employment_type": c.occupation, "income": c.monthly_income, "work_experience_years": c.work_experience_years, "years_in_business": c.years_in_business, "emergency_contacts": []},
        "risk_score": risk,
        "loan_trend": {"applications": len(loans), "pending_approvals": sum(x.status in {"assessment","review"} for x in loans), "due_repayments": sum(x.status == "repayment" for x in loans), "overdue_loans": sum(x.status == "overdue" for x in loans), "completed_loans": sum(x.status == "closed" for x in loans)},
        "tabs": ["profile", "contact", "bank_analysis", "kyc_employment", "risk_score"]
    }
