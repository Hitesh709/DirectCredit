...
def reporting(db:Session=Depends(get_db)):
    customers=db.query(CustomerRecord).all(); loans=db.query(LoanRecord).order_by(LoanRecord.id.desc()).all(); repayments=db.query(RepaymentRecord).all(); documents=db.query(DocumentRecord).all(); by=defaultdict(list)
    for r in repayments: by[r.loan_id].append(r)
    states={l.id:loan_state(l,by[l.id]) for l in loans}; dis=[l for l in loans if l.disbursed_amount or states[l.id] in {"active","overdue","repaid"}]; active=[l for l in loans if states[l.id]=="active"]; overdue=[l for l in loans if states[l.id]=="overdue"]
