...
# Explicit role boundary. A production admin principal must provide role=admin.
...
def customers(q: str | None = Query(default=None), db: Session = Depends(get_db)):
...
def documents(customer_id:int, db:Session=Depends(get_db)):
...
def loans(customer_id:int, db:Session=Depends(get_db)):
