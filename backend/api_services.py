...
def sync_customer_journey(customer_id: int, payload: dict, db: Session = Depends(get_db)):
    ...
    customer_fields = {
        ...
        "permanent_address": c.get("permanent_address"), "current_city": c.get("current_city"),
        "gender": c.get("gender"), "business_name": c.get("business_name"),
        "business_type": c.get("business_type"), "date_of_birth": c.get("date_of_birth"),
        "aadhaar_masked": c.get("aadhaar_masked"), "marital_status": c.get("marital_status"),
        "occupation": c.get("occupation"), "monthly_income": c.get("monthly_income", 0),
        "work_experience_years": c.get("work_experience_years", 0),
        "years_in_business": c.get("years_in_business", 0),
        "average_bank_balance": c.get("average_bank_balance", 0),
        "primary_bank": c.get("primary_bank"), "cibil_score": c.get("cibil_score", 0),
        "foir": c.get("foir", 0), "existing_emi": c.get("existing_emi", 0),
        "dependents": c.get("dependents", 0), "kyc_status": c.get("kyc_status", "pending"),
        "email_verified": c.get("email_verified", "pending"),
        "selfie_status": c.get("selfie_status", "pending")
    }
    ...
    for index, step in enumerate(steps, start=1):
        ...
        record = db.query(CustomerJourneyRecord).filter(
            CustomerJourneyRecord.customer_id == customer_id,
            CustomerJourneyRecord.step_key == key
        ).first()
        if not record:
            record = CustomerJourneyRecord(customer_id=customer_id, step_key=key)
            db.add(record)
        record.loan_id = loan.id
        record.step_number = int(step.get("step_number") or index)
        record.step_label = step.get("label") or key
    ...
    for doc in payload.get("documents") or []:
        ...
        existing = db.query(DocumentRecord).filter(
            DocumentRecord.customer_id == customer_id,
            DocumentRecord.file_name == file_name,
            DocumentRecord.document_type == document_type
        ).first()
        if not existing:
            existing = DocumentRecord(customer_id=customer_id, loan_id=loan.id, document_type=document_type, file_name=file_name)
            db.add(existing)
    ...
    for item in payload.get("repayments") or []:
        ...
        if not installment:
            continue
        repayment = db.query(RepaymentRecord).filter(
            RepaymentRecord.loan_id == loan.id,
            RepaymentRecord.installment == installment
        ).first()
        if not repayment:
            repayment = RepaymentRecord(loan_id=loan.id, installment=installment, due_date=str(item.get("due_date") or ""), due_amount=float(item.get("due_amount") or 0))
            db.add(repayment)
        repayment.due_date = str(item.get("due_date") or repayment.due_date)
        repayment.due_amount = float(item.get("due_amount") or repayment.due_amount or 0)
        repayment.paid_amount = float(item.get("paid_amount") or 0)
        repayment.status = item.get("status") or repayment.status
